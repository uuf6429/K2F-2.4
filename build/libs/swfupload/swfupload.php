<?php namespace \K2F\libs\swfupload; defined('K2F') or die;
	...
	class SwfUploadFile {
		/**
		 * @var string Filename of temporary upload file.
		 */
		protected $_path='';
		/**
		...
		/**
		 * @param string $newFile Filename to the new location.
		 * @return boolean Whether operation was a success or not.
		 */
		public function moveTo($newFile){
			return rename($this->_path, $newFile);
		}
		/**
		 * @param string $newFile Filename to the new location.
		 * @return boolean Whether operation was a success or not.
		 */
		public function copyTo($newFile){
			return rename($this->_path, $newFile);
		}
		/**
			...
			if($name!==null){
				if($index!==null){ // by index
					...
					if($_FILES[$name]['tmp_name'][$index]!='')
						$this->_path=$_FILES[$name]['tmp_name'][$index];
					$this->error=$_FILES[$name]['error'][$index];
				}else{ // the file
					$this->name=$_FILES[$name]['name'];
					if($_FILES[$name]['tmp_name']!='')
						$this->_path=$_FILES[$name]['tmp_name'];
					$this->error=$_FILES[$name]['error'];
				}
			}
		}
		public function __get($name){
			if($name=='data')return file_get_contents($this->_path);
		}
		public function __set($name, $value){
			if($name=='data')return file_put_contents($this->_path, $value);
		}
		/**
