<?php namespace \K2F\libs\websocket;
...
class WebSocketServer implements WebSocketObserver{
	...
	/**
	 * @var null|float|integer Interval between ticks: null=none, 0=immediate, anything else in seconds (as a float, accurate till useconds).
	 */
	...
	public function run(){
		...
		while(true){
			...
			$except = null;
			$sec = $this->interval===null ? null : abs(floor($this->interval));
			$usec = $this->interval===null ? null : abs(($this->interval - $sec) * 100000);
			
			if(@stream_select($changed,$write,$except,$sec,$usec) === false){
			...
			$this->debug('Performing tick event...');
	...
	protected function addConnectionToUriHandler(WebSocketConnection $user, $uri){
		...
		$resource = preg_split('/\//',$url['path'],0,PREG_SPLIT_NO_EMPTY);
