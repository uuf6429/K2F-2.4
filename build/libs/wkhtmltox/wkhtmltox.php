<?php namespace \K2F\libs\wkhtmltox; defined('K2F') or die;
	 ...
	 * @version
	 *   22/03/2010 - Initial implementation.
	 *   31/03/2010 - Fixed protected fields, IP instead domain, pages from multiple sources (thanks to Adam and Pierino).
	 ...
	 *   22/08/2012 - Trigerred errors now use proper error codes (instead of E_USER_NOTICE).
	 *   22/08/2012 - Fixed issue about calling ob_end_clean() without checking causing a warning.
	 */
	...
	abstract class WKCORE {
		...
		protected static function exe($bin){
			if(!self::$exe){
				...
				if(!file_exists(self::$exe)){
					trigger_error('WKIMG error: Could not find executable for the current platform.', E_USER_ERROR);
					return null;
				}
		...
		public function render(){
			...
			if($this->result['stdout']=='')
				trigger_error('WKHTMLTOX error: program returned empty result.', E_USER_ERROR);
			// handle execution return codes (only 0 means success).
			switch($this->result['return']){
				case 139:
					trigger_error('WKHTMLTOX system error 139: access violation (SEGFAULT), program terminated.', E_USER_ERROR);
					return false;
				case 126:
					trigger_error('WKHTMLTOX system error 126: target file not executable (execution failure).', E_USER_ERROR);
					return false;
				case 13:
					trigger_error('WKHTMLTOX system error 13: execution aborted (probably selinux-related).', E_USER_ERROR);
					return false;
				case 7:
					trigger_error('WKHTMLTOX system error 7: malformed executable (execution failure).', E_USER_ERROR);
					return false;
				case 3:
					trigger_error('WKHTMLTOX error 401: recieved "unauthorized access" for target url.', E_USER_ERROR);
					return false;
				case 2:
					trigger_error('WKHTMLTOX error 404: recieved "file not found" for target url.', E_USER_ERROR);
					return false;
				case 1:
					trigger_error('WKHTMLTOX system error 1: a generic error occured, possibly due to wrong parameters.', E_USER_ERROR);
					return false;
				case 0:
					return true;
				default:
					trigger_error('WKHTMLTOX error '.$this->result['return'].': Unidentified error code.', E_USER_ERROR);
					return false;
			}
		}
		/**
			...
			switch($mode){
				case self::OUT_DOWNLOAD:
					if(!@headers_sent()){
						...
						trigger_error('WKHTMLTOX error: HTTP headers were already sent.', E_USER_WARNING);
						return false;
					}
...
//						header('Last-Modified: '.@gmdate('D, d M Y H:i:s').' GMT');
				...
				default:
					trigger_error('WKHTMLTOX error: unsupported or unrecognized $mode value passed to output().', E_USER_ERROR);
					return false;
			}
		}
	}
